-- ============================================================
-- 補強：含日期的定期任務（w1/w2/w3/w4/a1/t1/t2...）同一 QuestID 不得重複
-- QuestID 格式：{prefix}|{YYYY-MM-DD}，以含 '|' 判斷為日期型任務
-- 防止客戶端 bug 或網路重試造成相同 QuestID 存入兩次
-- ============================================================
...
BEGIN
  ...
  IF p_quest_id IN ('q1', 'q1_dawn') THEN
    -- q1 與 q1_dawn 每日互斥（只能擇一），不計入任意定課上限
  ...
  ELSIF p_quest_id LIKE '%|%' THEN
    -- 日期型任務（w1/w2/w3/w4/a1/t1/t2 等，格式為 prefix|YYYY-MM-DD）
    -- 同一 QuestID 不允許重複記錄
    SELECT COUNT(*) INTO v_dup_count FROM "DailyLogs"
    WHERE "UserID" = p_user_id
      AND "QuestID" = p_quest_id;
    IF v_dup_count > 0 THEN
      RETURN jsonb_build_object('success', false, 'error', '此任務今日已完成。');
    END IF;
  END IF;
