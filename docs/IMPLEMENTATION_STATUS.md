...
**比對日期：2026-04-03（最後更新：2026-04-03）**
...
| b3-b7 額外加分 | ✅ | `app/actions/bonus.ts` 含三層審核流程 |
...
| 成員轉隊管理 UI | ✅ | AdminDashboard 成員管理區塊，`transferMember()` in `admin.ts` |
| 成員角色設定 UI | ✅ | AdminDashboard 成員管理區塊，`setMemberRole()` in `admin.ts` |
...
| 成員名冊管理（CSV 匯入 + UI 操作） | ✅ | CSV + AdminDashboard 成員管理區塊 |
| 罰款系統 | ✅ | `app/actions/fines.ts` |
| 定課分值編輯 & 啟停開關 | ✅ | AdminDashboard 定課分值 & 啟停管理區塊 |
...
| 電影裝飾圖示（膠卷、打板機等） | ✅ | `components/ui/FilmIcons.tsx`：膠卷圓盤/條紋/3D眼鏡/擴音器/打板機/放映機 SVG；Tab 導航列加圖示；Header 膠卷條紋分隔線；Login 頁面浮動裝飾 |

---

## 二、待完成 / 有落差項目 (Remaining Gaps)

### 視覺裝飾（低優先）

| # | 規格要求 | 現況 | 說明 |
|---|---------|------|------|
| 1 | 裝飾圖示：膠卷條紋、打板機、3D眼鏡、擴音器、膠卷圓盤 | 未實作 | 使用 Lucide 通用圖標，不影響功能 |
